...
public:
        ...
        for (int i = 0; i < 8; ++i) {
            ...
            if (newRow >= 0 && newRow < 8 && newCol >= 0 && newCol < 8) {
                ...
                if ((board[newIndex].type == EMPTY || board[newIndex].color != piece.color)) {
                    ...
                    temp.makeMove(pieceIndex, newIndex);
                    if(!isCheck(temp, piece.color)){
                        possibleMoves.push_back(newIndex);
                    }
                }
            }
    ...
    //     // Iterate through all the king's possible moves
    //     char opponentColor = (playerColor == 'W') ? 'B' : 'W';
    //     for (int i = 0; i < Total_tiles; i++) {
    //         if (toupper(chessboard.board[i].type) == KING && chessboard.board[i].color == playerColor) {
    //             chessboard.ComputeKingMoves(i, possibleMoves); // Example function, change as needed

    //             // Check if any of the king's moves can get it out of check
    //             for (int move : possibleMoves) {
    //                 // Simulate making the move and see if the king is still in check
    //                 ChessBoard tempBoard = chessboard; // Make a copy of the board
    //                 tempBoard.makeMove(i, move); // Make the move
    ...
    bool isValidMove(const string& move, int& fromTile, int& toTile) const {
        ...
        if (toupper(board[fromTile].type) == PAWN) {
            if (isValidPawnMove(fromTile, toTile, PawnMoveData)) {
                ...
                cout << "Invalid move for " << board[fromTile].type << endl;
                return false;
            }
        }
